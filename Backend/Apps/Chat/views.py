    ...
    def get(self, request, *args, **kwargs):
        response = {
            "unread_messages": Message.objects.filter((
                    Q(conversation__owner_id=self.request.user.pk) |
                    Q(conversation__opponent_id=self.request.user.pk)),
                read=False
            ).exclude(sender__username=self.request.user.username).count()
        }
